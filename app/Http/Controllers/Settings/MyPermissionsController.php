...
class MyPermissionsController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $items = MyPermission::query()->with(['menu'])->withTrashed();
            ...
            return datatables()->of($items)
                ...
                ->addColumn('action', function ($row) {
                    return renderActionButtons($row, [
                        'edit' => 'settings.my-permissions.edit',
                        'destroy' => 'settings.my-permissions.delete',
                    ]);
                })
        ...
        return view('settings.my_permissions.index', [
    ...
    public function create(MyPermission $item)
    {
        $data = [
            ...
            'new_route' => ['settings.my-permissions.create', 'New Permission'],
            'item' => $item,
            // 'apps' => MyApp::pluck('title', 'id'), // Removed - no longer needed
            ...
            if ($request->has('title')) {
                ...
                foreach ($request->get('title') as $rtitle) {
                    $thisInputs = [
                        ...
                        // 'app_id' => $request->get('app_id'), // Removed - no longer needed
