...
class DownloadResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'attachment_date' => optional($this->attachment_date)->format('d/m/Y'),
            'description' => $this->description,
            'status' => $status,

            'category_name' => $category ? $category->title : null,

            // department can be "name" in your tables; fallback to title if your model uses title
            'department_name' => $department ? ($department->name ?? $department->title) : null,

            'site' => $company ? $company->title : null,

            // keeping same key naming pattern as your sample resource
