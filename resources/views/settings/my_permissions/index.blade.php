                ...
                <div class="card-body">
                    <div class="table-responsive mb-4">
                        <table id="myDataTable" class="table table-bordered myDataTable"
                            data-route="{{ route('settings.my-permissions.list') }}"
                            raw-columns="name,slug,model,menu,show_in_menu,action">
                            <thead>
                                <tr>
                                    <th>Action</th>
                                    <th>Title</th>
    ...
    <script>
        $(function() {
            ...
            $('#myDataTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('settings.my-permissions.list') }}",
                    type: "GET",
                    data: function(d) {
                ...
                buttons: [
                    {
                        ...
                        buttons: [
                            ...
                            },
                            {
                                ...
                                filename: 'permissions-' + new Date().toISOString().slice(0, 10)
                            },
                            {
                                extend: 'excel',
                    ...
                    {
                        text: '<i class="bx bx-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">New Permission</span>',
                        className: 'btn btn-primary',
                        action: function() {
                            window.location.href = "{{ route('settings.my-permissions.create') }}";
                        }
                    }
                ],
                language: {
            ...
            $('.head-label').html('<h5 class="card-title mb-0">Permissions List</h5>');
        });
    </script>
@endpush
