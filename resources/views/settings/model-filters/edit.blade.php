    ...
    <div class="col-12 mb-4">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Edit Model Filter</h4>
            </div>
            <div class="card-body">
                ...
                <form action="{{ route('settings.model.filters.update', $filter->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                                ...
                                <div class="card-body">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="filter_for_type" name="filter_for_type" required>
                                            ...
                                            <option value="App\Models\User" {{ old('filter_for_type', $filter->filter_for_type) == 'App\Models\User' ? 'selected' : '' }}>User</option>
                                            <option value="App\Models\MyRole" {{ old('filter_for_type', $filter->filter_for_type) == 'App\Models\MyRole' ? 'selected' : '' }}>Role</option>
                                        </select>
                                    </div>
                                ...
                                <div class="card-body">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="model_type" name="model_type" required>
                                            ...
                                            @foreach($availableModels as $modelClass => $modelName)
                                                <option value="{{ $modelClass }}" {{ old('model_type', $filter->model_type) == $modelClass ? 'selected' : '' }}>
                                                    {{ $modelName }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="number" class="form-control" id="model_id" name="model_id" 
                                               value="{{ old('model_id', $filter->model_id) }}" placeholder="Leave empty for all instances">
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-check mb-3">
                                        <input class="form-check-input" type="checkbox" id="is_filter_by_user" 
                                               name="is_filter_by_user" value="1" {{ old('is_filter_by_user', $filter->is_filter_by_user) ? 'checked' : '' }}>
                                    ...
                                    <div class="form-check mb-3">
                                        <input class="form-check-input" type="checkbox" id="is_filter_by_company" 
                                               name="is_filter_by_company" value="1" {{ old('is_filter_by_company', $filter->is_filter_by_company) ? 'checked' : '' }}>
                                    ...
                                    <div class="form-check mb-3">
                                        <input class="form-check-input" type="checkbox" id="is_filter_by_section" 
                                               name="is_filter_by_section" value="1" {{ old('is_filter_by_section', $filter->is_filter_by_section) ? 'checked' : '' }}>
                                ...
                                <div class="col-md-6">
                                    <div class="form-check mb-3">
                                        <input class="form-check-input" type="checkbox" id="is_filter_by_unit" 
                                               name="is_filter_by_unit" value="1" {{ old('is_filter_by_unit', $filter->is_filter_by_unit) ? 'checked' : '' }}>
                                    ...
                                    <div class="form-check mb-3">
                                        <input class="form-check-input" type="checkbox" id="is_filter_by_status" 
                                               name="is_filter_by_status" value="1" {{ old('is_filter_by_status', $filter->is_filter_by_status) ? 'checked' : '' }}>
                                    ...
                                    <div class="form-check mb-3">
                                        <input class="form-check-input" type="checkbox" id="is_active" 
                                               name="is_active" value="1" {{ old('is_active', $filter->is_active) ? 'checked' : '' }}>
                    ...
                    <div class="row">
                        <div class="col-12">
                            <button type="submit" class="btn btn-primary">
                                <i class="bx bx-check"></i> Update Filter
                            </button>
...
$(document).ready(function() {
    // Pre-load entities based on existing filter_for_type
    loadEntities('{{ $filter->filter_for_type }}', '{{ $filter->filter_for_id }}');
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                data.forEach(function(entity) {
                    const isSelected = selectedId && entity.id == selectedId ? 'selected' : '';
                    entitySelect.append(`<option value="${entity.id}" ${isSelected}>${entity.title}</option>`);
                });
            },
            error: function() {
