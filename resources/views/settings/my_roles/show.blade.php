...
@push('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
                ...
                // Initialize fancytree for each tree
                $(".tree-checkbox-hierarchical").each(function() {
                    var $tree = $(this);

                    // Store which nodes should be selected before initialization
                    var selectedKeys = [];
                    $tree.find('li.selected').each(function() {
                        selectedKeys.push($(this).attr('id'));
                    });
                    ...
                    $tree.fancytree({
                        ...
                        init: function(event, data) {
                            // After tree is initialized, select nodes that were marked as selected
                            var tree = data.tree;
                            $.each(selectedKeys, function(index, key) {
...
@section('content')
                    ...
                    <div class="row">
                        ...
                        <div class="col-12">
                            ...
                            <div class="alert alert-info mt-3">
                                ...
                                <p>Please select permissions that you want to assign to this role. After you select all permissions, make sure to click "Save Permissions" button at the bottom of the page.</p>
                            </div>
    ...
    <form class="form-horizontal" id="permissions_assignment_form" method="POST" action="{{ route('settings.my-roles.role-permissions-save', \Illuminate\Support\Facades\Crypt::encrypt($item->id)) }}">
        @csrf
            ...
            <div class="card">
                ...
                <div class="card-body" style="max-height: 600px !important; min-height: 400px !important; overflow-y: auto;">
                    ...
                    <div class="tree-checkbox-hierarchical">
                        @php
                            if (!function_exists('renderMenuWithPermissionsRecursive')) {
                                function renderMenuWithPermissionsRecursive($menu, $assigned_perms_ids) {
                                    $html = '<li id="menu-' . $menu->id . '" rel="' . $menu->id . '" class="folder expanded">' . e($menu->title) . '<ul>';

                                    // Render Permissions
                                    foreach($menu->myPermissions as $perm) {
                                        $selectedClass = in_array($perm->id, $assigned_perms_ids) ? 'selected' : '';
                                        $html .= '<li id="perm-' . $perm->id . '" rel="' . $perm->id . '" class="' . $selectedClass . '">' . e($perm->name) . '</li>';
                                    }
                                    ...
                                    if ($children && $children->count() > 0) {
                                        foreach($children as $child) {
                                            $html .= renderMenuWithPermissionsRecursive($child, $assigned_perms_ids);
                                        }
                                    }
                        ...
                        <ul>
                            ...
                            @foreach($trees[0]->menus as $menu)
                                {!! renderMenuWithPermissionsRecursive($menu, $assigned_perms_ids) !!}
                            @endforeach
